...
| Dependency alerts | Dependency update policy exists for security patches, regular updates, and major upgrades. | TBD | YYYY-MM-DD | TBD | ADR, runbook, bot config, check-in schedule, or issue labels. | Before first release; after painful/stale dependency drift. | TBD |
